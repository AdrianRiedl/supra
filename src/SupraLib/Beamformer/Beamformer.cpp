...
namespace supra {
    ...
    Beamformer::Beamformer()
            : m_pRxBeamformerParameters(nullptr), m_pTransducer(nullptr), m_type(Beamformer::Linear),
              m_correctMatchingLayers(true), m_numScanlines{0, 0}, m_rxScanlineSubdivision{1, 1},
              m_numRxScanlines{0, 0}, m_maxApertureSize{0, 0}, m_txMaxApertureSize{0, 0}, m_txWindow(WindowRectangular),
              m_depth(0.0), m_txSteeringAngle{0, 0}, m_txSectorAngle{0.0, 0.0}, m_txFocusActive(false),
              m_txFocusDepth(0.0), m_txFocusWidth(0.0), m_rxFocusDepth(0.0), m_speedOfSound(1540.0),
              m_speedOfSoundMMperS(m_speedOfSound * 1000.0), m_numSamplesRecon(0), m_ready(false) {
    }

    Beamformer::Beamformer(const std::shared_ptr <Beamformer> bf)
            : m_txParameters(bf->m_txParameters), m_rxParameters(nullptr) // re-computed online
            , m_pRxBeamformerParameters(nullptr) // re-computed online
            , m_pTransducer(bf->m_pTransducer), m_type(bf->m_type),
              m_correctMatchingLayers(bf->m_correctMatchingLayers), m_numScanlines(bf->m_numScanlines),
              m_rxScanlineSubdivision(bf->m_rxScanlineSubdivision), m_numRxScanlines(bf->m_numRxScanlines),
              m_maxApertureSize(bf->m_maxApertureSize), m_txMaxApertureSize(bf->m_txMaxApertureSize),
              m_txWindow(bf->m_txWindow), m_txWindowParameter(bf->m_txWindowParameter), m_depth(bf->m_depth),
              m_txSteeringAngle(bf->m_txSteeringAngle), m_txSectorAngle(bf->m_txSectorAngle),
              m_txFocusActive(bf->m_txFocusActive), m_txFocusDepth(bf->m_txFocusDepth),
              m_txFocusWidth(bf->m_txFocusWidth), m_rxFocusDepth(bf->m_rxFocusDepth),
              m_speedOfSound(bf->m_speedOfSound), m_speedOfSoundMMperS(bf->m_speedOfSoundMMperS),
              m_numSamplesRecon(bf->m_numSamplesRecon), m_ready(false) {
    }
            ...
            if (m_type == Linear) {
                ...
                for (size_t scanlineIdx = 0; scanlineIdx < numScanlines; scanlineIdx++) {
                    ...
                    double scanlinePosition = firstElement.x +
                                              static_cast<double>(scanlineIdx) / (numScanlines - 1) *
                                              (lastElement.x - firstElement.x);

                    // the scanline position in terms of elementIndices
                    double scanlinePositionRelative = static_cast<double>(scanlineIdx) / (numScanlines - 1) *
                                                      (m_pTransducer->getNumElements() - 1);

                    rect2s activeAperture = computeAperture(elementLayout, m_maxApertureSize,
                                                            {scanlinePositionRelative, 0});
                    rect2s txAperture = computeAperture(elementLayout, m_txMaxApertureSize,
                                                        {scanlinePositionRelative, 0});

                    double scanlineAngle =
                            steerAngle - (sectorAngle / 2) + ((double) scanlineIdx / (numScanlines - 1) * sectorAngle);
                    m_txParameters[scanlineIdx] = getTxScanline3D(activeAperture, txAperture,
                                                                  vec2d{scanlinePosition, 0}, vec2d{scanlineAngle, 0});
                }
                ...
                rect2s activeAperture = computeAperture(elementLayout, m_maxApertureSize,
                                                        vec2d{(static_cast<double>(numElements) - 1) / 2, 0});
                rect2s txAperture = computeAperture(elementLayout, m_txMaxApertureSize,
                                                    vec2d{(static_cast<double>(numElements) - 1) / 2, 0});
                ...
                for (size_t scanlineIdx = 0; scanlineIdx < numScanlines; scanlineIdx++) {
                    ...
                    double scanlineAngle =
                            steerAngle - (sectorAngle / 2) + (scanlineIdx / (numScanlines - 1) * sectorAngle);
                    m_txParameters[scanlineIdx] = getTxScanline3D(activeAperture, txAperture, vec2{scanlineStartX, 0},
                                                                  vec2{scanlineAngle, 0});
                }
                ...
                for (size_t scanlineIdxY = 0; scanlineIdxY < m_numScanlines.y; scanlineIdxY++) {
                    for (size_t scanlineIdxX = 0; scanlineIdxX < m_numScanlines.x; scanlineIdxX++) {
                        ...
                        // the angle of the scanline
                        vec2 scanlineAngle = m_txSteeringAngle + (vec2{(double) scanlineIdxX, (double) scanlineIdxY} /
                                                                  (m_numScanlines - 1) - 0.5) * m_txSectorAngle;
                        m_txParameters[scanlinesDone] = getTxScanline3D(activeAperture, txAperture, scanlineStart,
                                                                        scanlineAngle);
                        scanlinesDone++;
                    }
                }
                ...
                for (size_t planewaveIdx = 0; planewaveIdx < numPlanewaves; planewaveIdx++) {
                    ...
                    auto planewaveAngle =
                            steerAngle - (sectorAngle / 2) + planewaveIdx * sectorAngle / (numPlanewaves - 1);
                    m_txParameters[planewaveIdx] = getTxScanline3D(activeAperture, txAperture,
                                                                   vec2d{scanlinePosition, 0},
                                                                   vec2d{planewaveAngle, 0});
                }
            } else {
            ...
            if (m_type != Planewave) {
                ...
                if (m_numScanlines.y > 1) {
                    for (size_t scanlineIdxY = 0; scanlineIdxY < (m_numScanlines.y - 1); scanlineIdxY++) {
                                ...
                                for (size_t rxScanIdxX = 0; rxScanIdxX < m_rxScanlineSubdivision.x; rxScanIdxX++) {
                                    ...
                                    ScanlineRxParameters3D interpolated =
                                            getRxScanline3DInterpolated(
                                                    ...
                                                    interp);
                                    //and store
                                    (*m_rxParameters)[scanlineIdxX * m_rxScanlineSubdivision.x + rxScanIdxX][
                                            scanlineIdxY * m_rxScanlineSubdivision.y + rxScanIdxY] =
                                            interpolated;
                                }
                            }
                        }
                        ...
                        (*m_rxParameters)[(m_numScanlines.x - 1) * m_rxScanlineSubdivision.x][scanlineIdxY *
                                                                                              m_rxScanlineSubdivision.y] =
                    ...
                    (*m_rxParameters)[(m_numScanlines.x - 1) * m_rxScanlineSubdivision.x][(m_numScanlines.y - 1) *
                                                                                          m_rxScanlineSubdivision.y] =
                            getRxScanline3D(txScanlineIdx, m_txParameters[txScanlineIdx]);
                    ...
                    (*m_rxParameters)[(m_numScanlines.x - 1) * m_rxScanlineSubdivision.x][0] = getRxScanline3D(
                            (m_numScanlines.x - 1), m_txParameters[(m_numScanlines.x - 1)]);
                }
            } else if (m_type == Planewave) {
                m_rxParameters->resize(m_numRxScanlines.x * m_numScanlines.x,
                                       vector<ScanlineRxParameters3D>(m_numRxScanlines.y * m_numScanlines.y));
                ...
                if (m_numScanlines.x > 1) {
                    ...
                    for (size_t planewaveIdx = 0; planewaveIdx < numPlanewaves; planewaveIdx++) {
                        auto planewaveAngle =
                                steerAngle - (sectorAngle / 2) + planewaveIdx * sectorAngle / (numPlanewaves - 1);
                        auto txScanline1 = getTxScanline3D(activeAperture, txAperture, vec2d{firstElement.x, 0},
                                                           vec2d{planewaveAngle, 0});
                        auto txScanline2 = getTxScanline3D(activeAperture, txAperture, vec2d{lastElement.x, 0},
                                                           vec2d{planewaveAngle, 0});
    ...
    ScanlineTxParameters3D Beamformer::getTxScanline3D(
        ...
        if (txAperture.begin.x == txAperture.end.x && txAperture.begin.y == txAperture.end.y) {
            cornerElements.push_back(std::tuple<vec2, vec2s, vec>({0.5, 0.5}, txAperture.begin, elementCenterpoints->at(
                    txAperture.begin.x + txAperture.begin.y * elementLayout.x)));
        } else if (txAperture.begin.x == txAperture.end.x) {
            cornerElements.push_back(std::tuple<vec2, vec2s, vec>({0.5, 0}, txAperture.begin, elementCenterpoints->at(
                    txAperture.begin.x + txAperture.begin.y * elementLayout.x)));
            cornerElements.push_back(std::tuple<vec2, vec2s, vec>({0.5, 1}, txAperture.end, elementCenterpoints->at(
                    txAperture.end.x + txAperture.end.y * elementLayout.x)));
        } else if (txAperture.begin.y == txAperture.end.y) {
            cornerElements.push_back(std::tuple<vec2, vec2s, vec>({0, 0.5}, txAperture.begin, elementCenterpoints->at(
                    txAperture.begin.x + txAperture.begin.y * elementLayout.x)));
            cornerElements.push_back(std::tuple<vec2, vec2s, vec>({1, 0.5}, txAperture.end, elementCenterpoints->at(
                    txAperture.end.x + txAperture.end.y * elementLayout.x)));
        } else {
            cornerElements.push_back(std::tuple<vec2, vec2s, vec>({0, 0}, {txAperture.begin.x, txAperture.begin.y},
                                                                  elementCenterpoints->at(txAperture.begin.x +
                                                                                          txAperture.begin.y *
                                                                                          elementLayout.x)));
            cornerElements.push_back(std::tuple<vec2, vec2s, vec>({0, 1}, {txAperture.begin.x, txAperture.end.y},
                                                                  elementCenterpoints->at(txAperture.begin.x +
                                                                                          txAperture.end.y *
                                                                                          elementLayout.x)));
            cornerElements.push_back(std::tuple<vec2, vec2s, vec>({1, 0}, {txAperture.end.x, txAperture.begin.y},
                                                                  elementCenterpoints->at(txAperture.end.x +
                                                                                          txAperture.begin.y *
                                                                                          elementLayout.x)));
            cornerElements.push_back(std::tuple<vec2, vec2s, vec>({1, 1}, {txAperture.end.x, txAperture.end.y},
                                                                  elementCenterpoints->at(txAperture.end.x +
                                                                                          txAperture.end.y *
                                                                                          elementLayout.x)));
            is3D = true;
        ...
        for (size_t activeElementIdxX = activeAperture.begin.x;
             activeElementIdxX <= activeAperture.end.x; activeElementIdxX++) {
            for (size_t activeElementIdxY = activeAperture.begin.y;
                 activeElementIdxY <= activeAperture.end.y; activeElementIdxY++) {
        ...
        for (size_t activeElementIdxX = txAperture.begin.x;
             activeElementIdxX <= txAperture.end.x; activeElementIdxX++) {
            for (size_t activeElementIdxY = txAperture.begin.y;
                 activeElementIdxY <= txAperture.end.y; activeElementIdxY++) {
        ...
        //compute the delays
        params.delays.resize(txAperture.end.x - txAperture.begin.x + 1,
                             vector<double>(txAperture.end.y - txAperture.begin.y + 1, 0));
        double maxDelay = 0.0;
        if (m_txFocusActive) {
            // with focus
            ...
            for (auto t : cornerElements) {
                ...
                vec elementToFocus =
                        scanlineStart3 + m_txFocusDepth * scanlineDir + focusPointFromFocusCenter - elementPos;
                double transitTime = m_pTransducer->computeTransitTime(elementIndex, elementToFocus,
                                                                       m_speedOfSoundMMperS, m_correctMatchingLayers);
            ...
            for (size_t activeElementIdxX = txAperture.begin.x;
                ...
                for (size_t activeElementIdxY = txAperture.begin.y;
                    ...
                    size_t localElementIdxY =
                            activeElementIdxY - txAperture.begin.y; // relative index (0 to 1) of element pos
                    vec2 relativeIndex =
                            vec2{static_cast<double>(localElementIdxX), static_cast<double>(localElementIdxY)}
                    ...
                    vec elementCenter = elementCenterpoints->at(
                            activeElementIdxX + activeElementIdxY * elementLayout.x);
                    ...
                    vec elementToFocus =
                            scanlineStart3 + m_txFocusDepth * scanlineDir + focusPointFromFocusCenter - elementCenter;

                    // calculate transit time from element to focus and get element delay to account for max transit time
                    double transitTime = m_pTransducer->computeTransitTime(vec2s{activeElementIdxX, activeElementIdxY},
                                                                           elementToFocus, m_speedOfSoundMMperS,
                                                                           m_correctMatchingLayers);
        ...
        params.weights.resize(txAperture.end.x - txAperture.begin.x + 1,
                              vector<double>(txAperture.end.y - txAperture.begin.y + 1, 0));
        for (size_t activeElementIdxX = txAperture.begin.x;
             activeElementIdxX <= txAperture.end.x; activeElementIdxX++) {
            for (size_t activeElementIdxY = txAperture.begin.y;
                ...
                double weight;
                if (is3D) {
                    weight = std::sqrt(win.get(static_cast<float>(elementRelativeDistance.x)) *
                                       win.get(static_cast<float>(elementRelativeDistance.y)));
                } else {
                    weight = win.get(static_cast<float>(elementRelativeDistance.x)) *
                             win.get(static_cast<float>(elementRelativeDistance.y));
                }
    ...
    ScanlineRxParameters3D Beamformer::getRxScanline3DInterpolated(
        ...
        for (auto txParam : rx.txParameters) {
            cornerElements.push_back(elementCenterpoints->at(
                    txParam.firstActiveElementIndex.x + txParam.firstActiveElementIndex.y * elementLayout.x));
            cornerElements.push_back(elementCenterpoints->at(
                    txParam.firstActiveElementIndex.x + txParam.lastActiveElementIndex.y * elementLayout.x));
            cornerElements.push_back(elementCenterpoints->at(
                    txParam.lastActiveElementIndex.x + txParam.firstActiveElementIndex.y * elementLayout.x));
            cornerElements.push_back(elementCenterpoints->at(
                    txParam.lastActiveElementIndex.x + txParam.lastActiveElementIndex.y * elementLayout.x));
        }
        vec2 maxDist = {0, 0};
