...
namespace supra {
BeamformingNode::BeamformingNode(tbb::flow::graph &graph, const std::string &nodeID, bool queueing) : AbstractNode(
        nodeID, queueing), m_lastSeenImageProperties(nullptr), m_beamformer(nullptr), m_lastSeenBeamformerParameters(
        nullptr) {
    if (queueing) {
        m_node = unique_ptr<NodeTypeQueueing>(
                new NodeTypeQueueing(graph, 1, [this](shared_ptr <RecordObject> inObj) -> shared_ptr <RecordObject> {
                    return checkTypeAndBeamform(inObj);
                }));
    } else {
        m_node = unique_ptr<NodeTypeQueueing>(
                new NodeTypeQueueing(graph, 1, [this](shared_ptr <RecordObject> inObj) -> shared_ptr <RecordObject> {
    ...
    m_valueRangeDictionary.set<string>("windowType", {"Rectangular", "Hann", "Hamming", "Gauss"}, "Rectangular",
                                       "RxWindow");
...
std::shared_ptr <USImage> BeamformingNode::beamformTemplated(std::shared_ptr<const USRawData> pRawData) {
    switch (m_outputType) {
        case supra::TypeInt16:
            ...
            return m_beamformer->performRxBeamforming<InputType, int16_t>(m_beamformerType, pRawData, m_fNumber,
                                                                          m_speedOfSoundMMperS, m_windowType,
                                                                          static_cast<WindowFunction::ElementType>(m_windowParameter),
                                                                          m_interpolateTransmits, m_additionalOffset);
            break;
        case supra::TypeFloat:
            logging::log_info("Calling performRxBeamforming at TypeFloat");
            // This function call links directly to the funcion performRxBeamforming in the corresponding RxBeamformer class.
            // See the files RxBeamformerCuda, RxBeamformerMV, RxBeamformerMVpcg!
            return m_beamformer->performRxBeamforming<InputType, float>(m_beamformerType, pRawData, m_fNumber,
                                                                        m_speedOfSoundMMperS, m_windowType,
                                                                        static_cast<WindowFunction::ElementType>(m_windowParameter),
                                                                        m_interpolateTransmits, m_additionalOffset);
            break;
...
shared_ptr <RecordObject> BeamformingNode::checkTypeAndBeamform(shared_ptr <RecordObject> inObj) {
    ...
    if (inObj->getType() == TypeUSRawData) {
        ...
        if (pRawData) {
            ...
            if (!m_lastSeenBeamformerParameters ||
            ...
            logging::log_error(
                    "BeamformingNode: could not cast object to USRawData type, is it in supported ElementType?");
        }
    }
    return pImageRF;
}
