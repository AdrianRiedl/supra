...
namespace supra
{
	TensorRtNode::TensorRtNode(tbb::flow::graph & graph, const std::string & nodeID, bool queueing)
		...
		, m_torchModule{ nullptr }
	{
		...
		m_callFrequency.setName("TensorRtNode");
	...
	void TensorRtNode::configurationChanged()
	{
	...
	void TensorRtNode::configurationEntryChanged(const std::string& configKey)
	{
	...
	std::shared_ptr<ContainerBase> TensorRtNode::processTemplateSelection(
		    ...
		    const std::string& finalLayout)
	{
		...
		case supra::TypeFloat:
			...
			break;
		default:
			logging::log_error("TensorRtNode: Output image type not supported");
			break;
		}
		return pOut;
	}

	shared_ptr<RecordObject> TensorRtNode::checkTypeAndProcess(shared_ptr<RecordObject> inObj)
	{
		...
		if (inObj)
		{
			...
			{
				logging::log_error("TensorRtNode: Model input class is configured as '", m_modelInputClass, "', but the recieved object is type '", inObj->getType(), "'");
			}
			else if (inObj->getType() == TypeUSImage)
			{
				...
				if (pInImage)
				{
					...
					{
						logging::log_warn("TensorRtNode: USImage->USRawData is not fully defined yet.");
					...
					case TypeFloat:
						...
						break;
					default:
						logging::log_error("TensorRtNode: Input image type not supported");
						break;
					}
					...
					logging::log_error("TensorRtNode: could not cast object to USImage type, although its type is TypeUSImage.");
				}
			}
			else if (inObj->getType() == TypeUSRawData)
			{
				...
				if (pInRawData)
				{
					...
					logging::log_error("TensorRtNode: could not cast object to USRawData type, although its type is TypeUSRawData.");
				}
			}
		}
		else {
			logging::log_error("TensorRtNode: Input was null.");
		}
		return pOut;
	}

	void TensorRtNode::loadModule() {
		m_torchModule = nullptr;
		if (m_modelFilename != "")
		{
			try {
				std::shared_ptr<TensorRtInference> module = std::make_shared<TensorRtInference>(m_modelFilename, m_inputNormalization, m_outputDenormalization);
				m_torchModule = module;
			}
			catch (std::runtime_error e)
			{
				logging::log_error("TensorRtNode: Exception (std::runtime_error) while creating TensorRtInference object from model '", m_modelFilename, "'");
				logging::log_error("TensorRtNode: ", e.what());
				m_torchModule = nullptr;
			}
			catch (std::exception e)
			{
				logging::log_error("TensorRtNode: Exception (std::exception) while creating TensorRtInference object from model '", m_modelFilename, "'");
				logging::log_error("TensorRtNode: ", e.what());
				m_torchModule = nullptr;
			}
		}
